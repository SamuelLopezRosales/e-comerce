...
$('.todo-list').sortable({
	...
	stop: function(event){
		for(var i=0; i<itemSlide.length; i++){
			// saco los id en el orden que quedaron
			var datos = new FormData();
...
/*====================================================================
AXTUALIZAR NOMBRE SLIDE
=====================================================================*/
// guardar los botones que existen de guardar slid e
...
for(var i=0; i<tipoSlide.length; i++){
	$("input[name='tipoSlide"+i+"']").on("ifChecked",function(){
		...
		$(posHorizontal[indice]).attr("tipoSlide",tipoSlide);
		...
		if(tipoSlide == "slideOpcion1"){
			...
			$(slideOpciones[indice]).children("img").css({"left": "","right":nuevaPosHImagen+"%"});
			$(guardarSlide[indice]).attr("estiloImgProductoLeft","");
			$(guardarSlide[indice]).attr("estiloImgProductoRight",nuevaPosHImagen);
			/* estilo texto slide */
			var posHTexto = $(slideOpciones[indice]).children(".textosSlide").css("right").replace(/px/,"");
			var nuevaPosHTexto = posHTexto*100/anchoSlide;
			$(slideOpciones[indice]).children(".textosSlide").css({"left": nuevaPosHTexto+"%","right":"","text-align":"left"});
			$(guardarSlide[indice]).attr("estiloTextoSlideLeft",nuevaPosHTexto);
			$(guardarSlide[indice]).attr("estiloTextoSlideRight","");

		}else{
			/* estilos imagenes */
			var posHImagen = $(slideOpciones[indice]).children("img").css("right").replace(/px/," ");
			var nuevaPosHImagen = posHImagen*100/anchoSlide;
			$(slideOpciones[indice]).children("img").css({"left": nuevaPosHImagen+"%","right":""});
			$(guardarSlide[indice]).attr("estiloImgProductoLeft",nuevaPosHImagen);
			$(guardarSlide[indice]).attr("estiloImgProductoRight","");
			/* estilos de ls textos */
			var posHTexto = $(slideOpciones[indice]).children(".textosSlide").css("left").replace(/px/,"");
			var nuevaPosHTexto = posHTexto*100/anchoSlide;
			$(slideOpciones[indice]).children(".textosSlide").css({"left": "","right":nuevaPosHTexto+"%","text-align":"right"});
			$(guardarSlide[indice]).attr("estiloTextoSlideLeft","");
			$(guardarSlide[indice]).attr("estiloTextoSlideRight",nuevaPosHTexto);
		}
		$(guardarSlide[indice]).attr("tipoSlide",tipoSlide);
	});
}

/*====================================================================
CAMBIAR IMAGEN FNDO DEL SLIDE
=====================================================================*/
...
/*====================================================================
CAMBIAR IMAGEN PRODUCTO DEL SLIDE
=====================================================================*/
...
$(".cambioColorTexto2").change(function(){
	...
	$(guardarSlide[indiceSlide]).attr("titulo2Color",color1);
});
...
/*====================================================================
CAMBIAR POSICION SLIDE VERTICAL
=====================================================================*/
...
/*====================================================================
CAMBIAR POSICION SLIDE HORIZONTAL
=====================================================================*/
var posVertical = new Slider('.posHorizontal'+i, {
	formatter: function(value){
...
/*====================================================================
CAMBIAR POSICION TEXTO VERTICAL
=====================================================================*/
for(var i =0; i<slideOpciones.length; i++){
var posVertical = new Slider('.posVerticalTexto'+i, {
	formatter: function(value){
...
CAMBIAR POSICION SLIDE HORIZONTAL
=====================================================================*/
var posVertical = new Slider('.posHorizontalTexto'+i, {
	formatter: function(value){
...
CAMBIAR POSICION SLIDE VERTICAL
=====================================================================*/
var anchoImagen = new Slider('.anchoTexto'+i, {
	formatter: function(value){
...
$('.urlSlide').change(function(){
	...
	if($(botonSlide[indiceSlide]).val() == ""){
		...
		swal({
			...
			text: "¡Debe escribir primero el texto del boton!",
			type: "error",
			confirmButtonText: "¡Cerrar!"
		});
	}else{
		$(slideOpciones[indiceSlide]).children('.textosSlide').children("a").attr("href", urlBoton);
		$(guardarSlide[indiceSlide]).attr("urlBoton",urlBoton);
	}
});
/*====================================================================
GUARDAR SLIDE
...
$(".guardarSlide").click(function(){
     ...
     // capturamos el cambio de imagen de producto //
	 ...
	 // ENVIAMOS EL CAMBIO DE IMAGEN DE FNDO
	 datos.append("imgFondo",imgFondo);
	 ...
	 $.ajax({
	 	...
	 	success: function(respuesta){
	 		...
	 		if(respuesta == "ok"){
	 			swal({
 					type: "success",
 					title: "El slide ha sido modificado correctamente",
 					showConfirmButton: true,
 					confirmButtonText: "Cerrar"
 				}).then((result)=>{
 					if(result.value){
