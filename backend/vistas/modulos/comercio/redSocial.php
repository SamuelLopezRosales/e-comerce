		...
		<?php
			foreach ($redSocial as $key => $value) {
				...
				echo '<div class="form-group row">
					...
					<div class="col-xs-2">';
					if($value["activo"] != 0){
						echo '<input type="checkbox" class="seleccionarRed" ruta="'.$value["url"].'" red="'.$value["red"].'" validarRed="'.$value["red"].'" estilo="'.$value["estilo"].'" checked>';
					}else{
						echo '<input type="checkbox" class="seleccionarRed" ruta="'.$value["url"].'" red="'.$value["red"].'" validarRed="'.$value["red"].'" estilo="'.$value["estilo"].'">';
					}
					echo '</div>
				</div>';
			}
		?>
		<input type="hidden" id="valorRedesSociales" value="<?php echo $plantilla["redesSociales"]; ?>" name="">
	</div>
